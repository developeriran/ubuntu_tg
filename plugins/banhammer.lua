...
local function kick_ban_res(extra, success, result) 
     ...
     local from_id = extra.from_id 
      ...
      elseif get_cmd == 'ban' then 
        ...
        send_large_msg(receiver, ' ♓️Username @'..member..' \n🆔 id ['..member_id..'] \n  بن شد و اجازه ورود به این  سوپرگروه رو ندارد📛 ') 
      ban_user(member_id, chat_id) 
      elseif get_cmd == 'unban' then 
        send_large_msg(receiver, ' ♓️Username @'..member..' \n🆔 id ['..member_id..'] \n آن بن شد و اجازه ورود به این  سوپرگروه رو دارد📛 ') 
        local hash =  'banned:'..chat_id 
        redis:srem(hash, member_id) 
        return 'یوزر '..user_id..' آن بن شد' 
      elseif get_cmd == 'banall' then 
        send_large_msg(receiver, ' ♓️Username @'..member..' \n🆔 id ['..member_id..'] \nگلوبال بن شد و اجازه ورود به سوپرگروه ها رو ندارد📛 ') 
      banall_user(member_id) 
      elseif get_cmd == 'unbanall' then 
        send_large_msg(receiver, ' ♓️Username @'..member..' \n🆔 id ['..member_id..'] \nگلوبال آن بن شد و اجازه ورود به سوپرگروه ها رو دارد📛 ') 
      ...
      if string.match(targetuser, '^%d+$') then 
        if tonumber(matches[2]) == tonumber(our_id) then 
           ...
           banall_user(targetuser) 
             return 'یوزر ['..user_id..' ] دیگر اجازه ورود به سوپرگروه های پلاس رو ندارد!' 
     else 
   local cbres_extra = { 
      chat_id = msg.to.id, 
      get_cmd = 'banall', 
    ...
    local chat_id = msg.to.id 
      if string.match(matches[2], '^%d+$') then 
        if tonumber(matches[2]) == tonumber(our_id) then 
           ...
           return ' ♓️Username @'..member..' \n🆔 id ['..member_id..'] \nگلوبال بن شد و اجازه ورود به سوپرگروه ها رو دارد📛 ' 
    else 
      local cbres_extra = { 
         chat_id = msg.to.id, 
         get_cmd = 'unbanall', 
